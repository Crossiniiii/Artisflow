...
const AIInsights: React.FC<AIInsightsProps> = ({ artworks, sales }) => {
    ...
    return [
      {
        ...
        icon: <Award className="text-neutral-600" size={24} />,
        color: 'bg-neutral-50'
      },
      {
        ...
        icon: <User className="text-neutral-600" size={24} />,
        color: 'bg-neutral-50'
      },
      {
        ...
        icon: <TrendingUp className="text-neutral-600" size={24} />,
        color: 'bg-neutral-50'
      },
      {
        ...
        icon: <AlertCircle className="text-neutral-600" size={24} />,
        color: 'bg-neutral-50'
      },
      {
        ...
        icon: <Sparkles className="text-neutral-600" size={24} />,
        color: 'bg-neutral-50'
      }
    ];
  }, [artworks, sales]);
  ...
  return (
    <div className="bg-white p-8 rounded-[3rem] shadow-sm border border-neutral-200">
      <div className="flex items-center space-x-3 mb-8">
        <div className="p-3 bg-neutral-900 rounded-2xl shadow-lg shadow-neutral-200">
          <Sparkles className="text-white" size={24} />
        </div>
        <div>
          <h2 className="text-2xl font-black text-neutral-900 tracking-tight">AI Data Analysis</h2>
          <p className="text-sm text-neutral-500">Automated insights based on your inventory patterns.</p>
        </div>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {insights.map((insight, idx) => (
          <div key={idx} className={`${insight.color} p-6 rounded-[2rem] border border-neutral-100/50 hover:shadow-md transition-shadow`}>
            <div className="flex justify-between items-start mb-4">
              <div className="p-3 bg-white rounded-xl shadow-sm">
                {insight.icon}
              </div>
              <span className="px-3 py-1 bg-white/60 rounded-full text-[10px] font-black uppercase tracking-widest text-neutral-500">
                Insight
              </span>
            </div>
            <h3 className="text-3xl font-black text-neutral-900 mb-1">{insight.value}</h3>
            <p className="text-sm font-bold text-neutral-700 mb-1">{insight.title}</p>
