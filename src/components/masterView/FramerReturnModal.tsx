...
import { Artwork, ArtworkStatus, FramerRecord } from '../../types';
import { compressImage } from '../../utils/imageUtils';

interface FramerReturnModalProps {
  artwork: Artwork;
  activeFramerRecord: FramerRecord | null;
  branches: string[];
  onReturnFromFramer?: (id: string, branch: string, returnDate: string, remarks?: string) => Promise<boolean | void> | boolean | void;
...
export const FramerReturnModal: React.FC<FramerReturnModalProps> = ({
  artwork,
  activeFramerRecord,
  branches,
  onReturnFromFramer,
  ...
  const [returnBranch, setReturnBranch] = useState(activeFramerRecord?.artworkSnapshot?.currentBranch || branches[0] || '');
    ...
    <Modal
      ...
      footer={
        <div className="flex gap-3 justify-end items-center w-full">
          ...
          <button
            onClick={async () => {
              if (activeFramerRecord && onReturnFromFramer && onTransfer && returnRemarks.trim() && handlingAgentName.trim()) {
                ...
                const success = await wrapAction(async () => {
                  ...
                  if (returnStrategy === 'manual') {
                    ...
                    const returnResult = await onReturnFromFramer(activeFramerRecord.id, originalBranch || returnBranch, now, fullRemarks);
                    if (returnResult === false) return;
                    await onTransfer(activeFramerRecord.artworkId, returnBranch, { itdrUrl: returnItdrUrl }, fullRemarks);
                  } else {
                    const returnResult = await onReturnFromFramer(activeFramerRecord.id, returnBranch, now, fullRemarks);
                    if (returnResult === false) return;
                  }
