...
const ApprovalsPage: React.FC<ApprovalsPageProps> = (props) => {
  const [activeTab, setActiveTab] = useState<'sales' | 'payments' | 'deliveries'>('sales');
  ...
  const canAccessPayments = (props.userPermissions?.accessibleTabs && Array.isArray(props.userPermissions.accessibleTabs))
    ...
    : true;
  const canAccessDeliveries = props.currentUser?.role === 'ADMIN' || 
                             props.userPermissions?.accessibleTabs?.includes('approvals') || 
                             props.userPermissions?.accessibleTabs?.includes('delivery-requests');

  // Auto-switch if only one is accessible
  React.useEffect(() => {
    if (!canAccessSales && canAccessPayments) {
      setActiveTab('payments');
    } else if (!canAccessSales && !canAccessPayments && canAccessDeliveries) {
      setActiveTab('deliveries');
    }
  }, [canAccessSales, canAccessPayments, canAccessDeliveries]);
  ...
  const pendingDeliveriesCount = props.sales.filter(s => 
    ...
    <div className="max-w-[1600px] mx-auto w-full space-y-6">
      ...
      <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-6 pb-8 border-b border-slate-200">
        <div className="space-y-1">
          <h1 className="text-4xl font-black text-slate-900 tracking-tight">Approvals Center</h1>
          <p className="text-sm font-medium text-slate-500">Manage administrative validations, financial verifications, and logistics</p>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-2 p-1.5 bg-slate-100/80 rounded-xl border border-slate-200/60 backdrop-blur-md w-fit">
          {canAccessSales && (
            ...
            >
              ...
              Sales
              {pendingSalesCount > 0 && (
                <span className={`ml-1 px-1.5 py-0.5 rounded-md text-[9px] font-black uppercase tracking-widest animate-pulse ${
                  activeTab === 'sales' ? 'bg-blue-100 text-blue-700' : 'bg-rose-600 text-white shadow-sm shadow-rose-200'
                }`}>
                  {pendingSalesCount}
                </span>
              )}
          ...
          {canAccessPayments && (
            ...
            >
              ...
              {pendingPaymentsCount > 0 && (
                <span className={`ml-1 px-1.5 py-0.5 rounded-md text-[9px] font-black uppercase tracking-widest animate-pulse ${
                  activeTab === 'payments' ? 'bg-blue-100 text-blue-700' : 'bg-rose-600 text-white shadow-sm shadow-rose-200'
                }`}>
          ...
          {canAccessDeliveries && (
            ...
            >
              ...
              {pendingDeliveriesCount > 0 && (
                <span className={`ml-1 px-1.5 py-0.5 rounded-md text-[9px] font-black uppercase tracking-widest animate-pulse ${
                  activeTab === 'deliveries' ? 'bg-blue-100 text-blue-700' : 'bg-rose-600 text-white shadow-sm shadow-rose-200'
                }`}>
      ...
      <div className="min-h-[600px]">
        <AnimatePresence mode="wait">
          ...
          ) : activeTab === 'deliveries' && canAccessDeliveries ? (
            <motion.div
              key="deliveries"
